// YouTube vid: https://www.youtube.com/watch?v=Rs_rAxEsAvI
// stopped at 1:00:00
// just r click index.html and click run with live server to use
...
function animate(time) {
    ...
    // find the car whose y value = min y value of all cars
    // the cars.map(c=>c.y) creates a new array with only the y values
    // ... "spreads" values because min cannot work with arrays
    bestCar = cars.find(
        c=>c.y==Math.min(
            ...cars.map(c=>c.y)
        ));
    ...
    requestAnimationFrame(animate); // requests animate method again and again, gives illusion of movement
}